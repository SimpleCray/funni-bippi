...
export class MatchingService implements OnModuleInit {
  ...
  async joinQueue(entry: { userId: string; socketId: string; gender: Gender }): Promise<void> {
    const { userId, socketId, gender } = entry
    const payload: QueueEntry = { userId, socketId, gender, joinedAt: Date.now() }
    ...
    if (!match) {
      const timer = setTimeout(() => this.onTimeout(userId, socketId), MATCH_TIMEOUT_MS)
  ...
  private async createMatch(user1: QueueEntry, user2: QueueEntry): Promise<void> {
    ...
    this.logger.log(`Match created: room ${roomId} (${user1.userId} ↔ ${user2.userId})`)
  }
