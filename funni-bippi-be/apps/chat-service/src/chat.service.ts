...
export class ChatService implements OnModuleInit {
  ...
  async handleMatchFound(payload: MatchFoundPayload): Promise<void> {
    const { roomId, user1, user2, strangerForUser1, strangerForUser2 } =
      payload;
  ...
  async handleUserLeft(data: {
    ...
    const roomId =
      data.roomId ?? (await this.redis.get(`userRoom:${data.userId}`));
    if (!roomId) return;
    ...
    const partnerSocketId =
      room.user1SocketId === data.socketId
        ? room.user2SocketId
        : room.user1SocketId;
    const partnerId =
      room.user1Id === data.userId ? room.user2Id : room.user1Id;
