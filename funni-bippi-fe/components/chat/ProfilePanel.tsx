...
export function ProfilePanel({ stranger, icebreakers, collapsed, onNext, onReport, onUseIce }: ProfilePanelProps) {
  return (
        ...
        <div className="panel-card">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', marginTop: 14 }}>
            {stranger.interests.map((it, i) => (
              <span
                key={i}
                style={{ fontSize: 12, fontWeight: 600, padding: '5px 11px', borderRadius: 99, background: 'var(--accent-soft)', color: 'var(--accent-strong)' }}
              >
                {it}
              </span>
            ))}
            ...
            {icebreakers.map((ib, i) => (
              <button key={i} className="ice-card" onClick={() => onUseIce(ib.parts.join(''))}>
                ...
                <span className="ice-text">
                  {ib.parts[0]}<b>{ib.parts[1]}</b>{ib.parts[2]}
                </span>
              </button>
            ))}
          </div>
