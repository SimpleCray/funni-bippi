...
export function MobileChatScreen({
  ...
  onSend, onTyping, onReact, onNext, onReport, onImageUpload, openSettings,
      ...
      {tab === 'chat' ? (
        <>
          ...
          {icebreakers.length > 0 && messages.length === 0 && (
            <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '6px 12px', flexShrink: 0, scrollbarWidth: 'none' }}>
              {icebreakers.map((ib, i) => (
                <button
                  key={i}
                  onClick={() => onSend(ib.parts.join(''))}
                  style={{
                    flexShrink: 0, fontSize: 12.5, fontWeight: 600, padding: '7px 13px',
                    borderRadius: 99, background: 'var(--bg-sunken)', color: 'var(--text-soft)',
          ...
          <div className="m-composer">
            <ComposerBar onSend={onSend} onTyping={onTyping} onImageUpload={onImageUpload} compact />
          </div>
        </>
      ) : (
        <div className="m-msgs scroll" style={{ alignItems: 'center', paddingTop: 32, gap: 16 }}>
          {stranger && (
            <div className="panel-card" style={{ width: '90%', maxWidth: 360 }}>
              <div className="big-ava"><Avatar stranger={stranger} size={80} online /></div>
          ...
          <div style={{ display: 'flex', gap: 9, width: '90%', maxWidth: 360 }}>
            <button className="btn btn-soft" style={{ flex: 1, justifyContent: 'center' }} onClick={onNext}>
              <IcShuffle size={17} /> Next
            </button>
            <button className="btn btn-danger" style={{ flex: 1, justifyContent: 'center' }} onClick={onReport}>
